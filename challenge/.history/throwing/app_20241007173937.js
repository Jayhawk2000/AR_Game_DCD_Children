...
class PoseDetector {
  ...
  checkArmBehindHead(landmarks) {
    ...
    return (leftWrist.x < head.x && leftWrist.y < head.y) || 
  ...
  checkThrowingPose(landmarks) {
    ...
    if (!this.throwingStarted) {
      ...
      this.initialWristPosition = leftWrist.x < rightWrist.x ? leftWrist : rightWrist;
  ...
  calculateThrowingScore(landmarks) {
    const throwingWrist = landmarks[15].x > landmarks[16].x ? landmarks[15] : landmarks[16];
    const throwDuration = (Date.now() - this.throwStartTime) / 1000; // 投掷时间（秒）

    const horizontalDistance = Math.abs(throwingWrist.x - this.initialWristPosition.x);
...
function showResultPage(totalScore, throwingScore, footStepScore) {
  ...
  returnButton.onclick = () => {
    window.location.href = '../index.html'; 佳灏
  };
}
