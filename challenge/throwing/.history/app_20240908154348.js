...
function calculateTrunkStability(currentLandmarks) {
  if (!initialPosePosition || !isMeasuringStability) {
    return 10; // Default to max score if we don't have initial position
  }
  ...
  const noseDisplacement = calculateDisplacement(initialPosePosition.nose, currentLandmarks[0]);
  const leftShoulderDisplacement = calculateDisplacement(initialPosePosition.leftShoulder, currentLandmarks[11]);
  const rightShoulderDisplacement = calculateDisplacement(initialPosePosition.rightShoulder, currentLandmarks[12]);
  const leftHipDisplacement = calculateDisplacement(initialPosePosition.leftHip, currentLandmarks[23]);
  const rightHipDisplacement = calculateDisplacement(initialPosePosition.rightHip, currentLandmarks[24]);

  // Increase the weight of head movement
  const headWeight = 0.5;
  const bodyWeight = 0.5;

  const headScore = calculateHeadScore(noseDisplacement);
  const bodyScore = calculateBodyScore(leftShoulderDisplacement, rightShoulderDisplacement, leftHipDisplacement, rightHipDisplacement);
...
function calculateHeadScore(displacement) {
  ...
  if (displacement < 0.10) return 1;
  return 0;
}

function calculateBodyScore(leftShoulder, rightShoulder, leftHip, rightHip) {
  const avgDisplacement = (leftShoulder + rightShoulder + leftHip + rightHip) / 4;
  ...
  if (avgDisplacement < 0.10) return 6;
  ...
  if (avgDisplacement < 0.20) return 1;
  return 0;
}
...
function getTrunkFeedback(score) {
  if (score >= 9)
    return "Excellent trunk stability! You maintained a very steady posture.";
  if (score >= 7)
    return "Good trunk stability. You had minimal movement throughout the pose.";
  if (score >= 5)
    return "Moderate trunk stability. Try to minimize swaying and keep your upper body more steady.";
  return "Work on maintaining a stable posture. Focus on keeping your upper body as still as possible.";
}
