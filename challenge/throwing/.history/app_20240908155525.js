...
function calculateTrunkStability() {
  ...
  for (let i = 1; i < posePositions.length; i++) {
    ...
    const leftShoulderDisp = calculateDisplacement(
      prev.leftShoulder,
      current.leftShoulder
    );
    const rightShoulderDisp = calculateDisplacement(
      prev.rightShoulder,
    ...
    totalBodyDisplacement +=
      (leftShoulderDisp + rightShoulderDisp + leftHipDisp + rightHipDisp) / 4;
  }

  const avgHeadDisplacement =
    totalHeadDisplacement / (posePositions.length - 1);
  const avgBodyDisplacement =
    totalBodyDisplacement / (posePositions.length - 1);
...
function checkBalancePose(landmarks) {
  ...
  if (oneFootRaised && (leftArmStatus.status === "raised" || rightArmStatus.status === "raised")) {
    ...
    initialPose = null;
    maxDisplacement = 0;
    resetPoseDetection(oneFootRaised, leftArmStatus.status === "raised" || rightArmStatus.status === "raised");
  }
}
