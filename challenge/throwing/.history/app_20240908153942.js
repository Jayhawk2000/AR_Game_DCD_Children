...
function calculateTrunkStability(currentLandmarks) {
  ...
  const noseDisplacement = calculateDisplacement(
    initialPosePosition.nose,
    currentLandmarks[0]
  );
  const leftShoulderDisplacement = calculateDisplacement(
    initialPosePosition.leftShoulder,
    currentLandmarks[11]
  );
  const rightShoulderDisplacement = calculateDisplacement(
    initialPosePosition.rightShoulder,
    currentLandmarks[12]
  );
  const leftHipDisplacement = calculateDisplacement(
    initialPosePosition.leftHip,
    currentLandmarks[23]
  );
  const rightHipDisplacement = calculateDisplacement(
    initialPosePosition.rightHip,
    currentLandmarks[24]
  );

  const totalDisplacement =
    noseDisplacement +
    leftShoulderDisplacement +
    rightShoulderDisplacement +
    leftHipDisplacement +
    rightHipDisplacement;

  // Scoring based on total displacement
  if (totalDisplacement < 0.05) {
    return 10; // Excellent stability
  } else if (totalDisplacement < 0.1) {
    return 9;
  } else if (totalDisplacement < 0.15) {
    return 8;
  } else if (totalDisplacement < 0.2) {
    return 7;
  } else if (totalDisplacement < 0.25) {
    return 6;
  } else {
    return 5; // Minimum score for larger displacements
  }
}
