...
function checkTrunkStability(currentShoulders, currentHips, prevShoulders, prevHips) {
  if (!prevShoulders || !prevHips) return 10; // 如果是第一帧，给予满分

  const shoulderMovement = calculateDistance(
    {x: (currentShoulders.left.x + currentShoulders.right.x) / 2, y: (currentShoulders.left.y + currentShoulders.right.y) / 2},
    {x: (prevShoulders.left.x + prevShoulders.right.x) / 2, y: (prevShoulders.left.y + prevShoulders.right.y) / 2}
  );

  const hipMovement = calculateDistance(
    {x: (currentHips.left.x + currentHips.right.x) / 2, y: (currentHips.left.y + currentHips.right.y) / 2},
    {x: (prevHips.left.x + prevHips.right.x) / 2, y: (prevHips.left.y + prevHips.right.y) / 2}
  );
  ...
  if (totalMovement <= 0.10) return 6;
