...
function checkBalancePose(landmarks) {
  ...
  if (oneFootRaised && leftArmStatus === "raised" && rightArmStatus === "raised" && headForward && trunkStable) {
    if (!balancePoseDetected) {
      ...
      const resultData = {
        ...
        armFeedback: getArmFeedback(armStabilityScore, leftWrist, rightWrist, leftShoulder, rightShoulder),
    ...
    resetPoseDetection(oneFootRaised, leftArmStatus, rightArmStatus, headForward);
  }
}
...
function checkTrunkStability(leftShoulder, rightShoulder, leftHip, rightHip) {
  ...
  // 不在检测阶段显示任何信息，只影响打分
  trunkStabilityScore = trunkStable ? trunkStabilityScore : trunkStabilityScore - 1;
...
function getArmFeedback(score, leftWrist, rightWrist, leftShoulder, rightShoulder) {
  ...
  if (score >= 9 && leftArmExtended && rightArmExtended) return "手臂稳定且完全伸展!";
...
function resetPoseDetection(oneFootRaised, leftArmStatus, rightArmStatus, headForward) {
  footInfo.textContent = oneFootRaised ? "" : "请抬高一只脚。";
  armInfo.textContent = (leftArmStatus === "raised" && rightArmStatus === "raised") ? "" : "请抬起手臂。";
