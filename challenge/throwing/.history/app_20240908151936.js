...
function checkBalancePose(landmarks) {
  ...
  if (
    ...
    if (!balancePoseDetected) {
      ...
      const armScore = (armStabilityScore + armExtensionScore) / 2;
...
function updateStabilityScores(
  ...
  if (prevWristPositions.left && prevWristPositions.right) {
    ...
    // Less strict scoring for arm stability
    armStabilityScore = Math.max(0, 10 - totalMovement * 50);
  }
  ...
  // Less strict scoring for trunk stability
  trunkStabilityScore = Math.max(0, 10 - trunkMovement * 100);
...
function getArmFeedback(score) {
  if (score >= 9) return "Excellent arm stability and position!";
  if (score >= 7) return "Good arm stability and position, minor improvements possible.";
  if (score >= 5) return "Moderate arm performance, focus on stability and position.";
  return "Keep working on arm stability and position.";
}
