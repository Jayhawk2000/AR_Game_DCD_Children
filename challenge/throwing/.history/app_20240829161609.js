...
function checkBalancePose(landmarks) {
  ...
  if (
    ...
    if (!balancePoseDetected) {
      ...
      info.textContent = "Hold the pose steady!";
    } else if (currentTime - balancePoseStartTime >= BALANCE_POSE_DURATION) {
      info.textContent = "Great balance! You held it for 3 seconds!";
      setTimeout(() => {
        info.textContent = "Try another pose!";
    ...
    if (!oneFootRaised) {
      ...
      info.textContent = "Raise both arms higher.";
      ...
      info.textContent = "Adjust your pose to hold it steady.";
    }
  }
  ...
  if (leftArmStatus === "excellent" && rightArmStatus === "excellent") {
    ...
    info.textContent += "\nGood arm position! Keep them steady.";
  }
}
