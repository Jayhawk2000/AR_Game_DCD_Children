...
function checkBalancePose(landmarks, width, height) {
  ...
  const oneFootRaised = Math.abs(leftAnkle.position.y - rightAnkle.position.y) > 0.05 * height;
...
function checkArmRaised(shoulder, elbow, wrist) {
  return (elbow.position.y < shoulder.position.y) && (wrist.position.y < elbow.position.y);
}
