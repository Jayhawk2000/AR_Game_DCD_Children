...
async function detectPose(imageData) {
  ...
  if (poses.length > 0) {
    ...
    const result = checkBalancePose(pose, imageData.width, imageData.height);
    return result;
  } else {
    return { error: '未检测到姿势' };
  }
  ...
  if (oneFootRaised && (leftArmStatus.status === 'raised' || rightArmStatus.status === 'raised')) {
    if (!initialPose) {
      ...
      if (currentTime - balancePoseStartTime >= BALANCE_POSE_DURATION) {
        ...
        return { message: `保持姿势 ${remainingTime} 秒`, isBalancePose: true };
      }
    }
  } else {
    initialPose = null;
    return { 
      message: oneFootRaised ? "请抬起手臂" : "请抬起一只脚",
      isBalancePose: false
    };
  }
}
