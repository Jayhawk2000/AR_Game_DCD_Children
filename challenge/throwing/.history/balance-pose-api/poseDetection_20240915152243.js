...
async function processFrame(imageData) {
  try {
    ...
    const tensorInput = tf.browser.fromPixels(image.bitmap);

    const results = await runPoseDetection(tensorInput, image.bitmap.width, image.bitmap.height);

    tensorInput.dispose();

    if (results.poseLandmarks) {
      const result = checkBalancePose(results.poseLandmarks, image.bitmap.width, image.bitmap.height);
      
      // 绘制关键点和骨架
      drawResults(image, results.poseLandmarks);
      
      const outputBuffer = await image.getBufferAsync(Jimp.MIME_JPEG);
      const outputImageData = `data:image/jpeg;base64,${outputBuffer.toString('base64')}`;
...
async function runPoseDetection(tensorInput, width, height) {
...
function drawResults(image, keypoints) {
  // 使用 Jimp 绘制关键点和骨架
  // ... 实现绘制逻辑 ...
}
