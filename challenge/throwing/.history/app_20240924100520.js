...
let jumpStartTime = 0;
const JUMP_PREPARE_DURATION = 3000; // 3秒倒计时准备
const MIN_JUMP_HEIGHT = 0.2; // 最小跳跃高度
let countdownTimer = 5; // 倒计时5秒
...
function detectJumpHeight(landmarks) {
  const initialY = landmarks[27].y; // 左脚踝的Y坐标
  const currentY = landmarks[27].y;
...
function checkFeetApartOnLanding(landmarks) {
  const leftAnkle = landmarks[27];
  const rightAnkle = landmarks[28];
  return Math.abs(leftAnkle.x - rightAnkle.x) > 0.1; // 检查落地时双脚是否张开
}
...
function updateInfoDisplay() {
  info.textContent = `Current phase: jumping`;
}
