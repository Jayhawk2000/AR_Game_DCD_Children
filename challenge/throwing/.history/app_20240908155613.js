...
function calculateTrunkStability() {
  // Higher displacement results in lower score
  ...
  if (maxDisplacement < 0.07) return 4;
  if (maxDisplacement < 0.08) return 3;
  if (maxDisplacement < 0.09) return 2;
  if (maxDisplacement < 0.1) return 1;
  return 0; // Very unstable
}

// Update the getTrunkFeedback function
function getTrunkFeedback(score) {
  ...
  if (score >= 6) return "Good stability. Some slight movement, but overall well maintained.";
  if (score >= 4) return "Fair stability. Try to reduce your body's movement more.";
  if (score >= 2) return "Needs improvement. Focus on keeping your body as still as possible.";
