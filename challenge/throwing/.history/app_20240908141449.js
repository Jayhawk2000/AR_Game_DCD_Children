...
function checkArmRaised(shoulder, elbow, wrist) {
  ...
  return {
    ...
    status: angleScore > 0 ? "raised" : "lowered",
  };
}

function checkArmStability(currentWrist, prevWrist) {
  if (!prevWrist) return 10; // 如果是第一帧，给予满分

  const movement = calculateDistance(currentWrist, prevWrist);

  // 假设移动 0.01 或以下被认为是轻微晃动
  if (movement <= 0.01) return 10;
  if (movement <= 0.02) return 9;
  if (movement <= 0.03) return 8;
  if (movement <= 0.04) return 7;
  if (movement <= 0.05) return 6;
  return Math.max(5, 10 - movement * 100); // 确保最低分为 5
}
...
function checkBalancePose(landmarks) {
  ...
  const leftArmStability = checkArmStability(leftWrist, prevWristPositions.left);
  const rightArmStability = checkArmStability(rightWrist, prevWristPositions.right);
  const trunkStability = checkTrunkStability(
    {left: leftShoulder, right: rightShoulder},
    {left: leftHip, right: rightHip},
    prevShoulderPositions,
    prevHipPositions
  );

  prevWristPositions = { left: leftWrist, right: rightWrist };
  prevShoulderPositions = { left: leftShoulder, right: rightShoulder };
  prevHipPositions = { left: leftHip, right: rightHip };

  const currentTime = Date.now();

  if (oneFootRaised && (leftArmStatus.status === "raised" || rightArmStatus.status === "raised") && headForward) {
    if (!balancePoseDetected) {
      ...
      const leftArmScore = (leftArmStatus.angleScore + leftArmStability) / 2;
      const rightArmScore = (rightArmStatus.angleScore + rightArmStability) / 2;
      const armScore = Math.max(leftArmScore, rightArmScore);
      const totalScore = (armScore + trunkStability) / 2;
      const resultData = {
        armScore: armScore,
        trunkScore: trunkStability,
        totalScore: totalScore,
        armFeedback: getArmFeedback(armScore),
        trunkFeedback: getTrunkFeedback(trunkStability)
      };
    ...
    resetPoseDetection(oneFootRaised, leftArmStatus.status === "raised" || rightArmStatus.status === "raised", headForward);
  }
}
...
function checkTrunkStability(
  currentShoulders,
  currentHips,
  prevShoulders,
  prevHips
) {
  if (!prevShoulders || !prevHips) return 10; // 如果是第一帧，给予满分

  const shoulderMovement = calculateDistance(
    {
      x: (currentShoulders.left.x + currentShoulders.right.x) / 2,
      y: (currentShoulders.left.y + currentShoulders.right.y) / 2,
    },
    {
      x: (prevShoulders.left.x + prevShoulders.right.x) / 2,
      y: (prevShoulders.left.y + prevShoulders.right.y) / 2,
    }
  );

  const hipMovement = calculateDistance(
    {
      x: (currentHips.left.x + currentHips.right.x) / 2,
      y: (currentHips.left.y + currentHips.right.y) / 2,
    },
    {
      x: (prevHips.left.x + prevHips.right.x) / 2,
      y: (prevHips.left.y + prevHips.right.y) / 2,
    }
  );

  const totalMovement = shoulderMovement + hipMovement;

  // 假设总移动 0.02 或以下被认为是轻微晃动
  if (totalMovement <= 0.02) return 10;
  if (totalMovement <= 0.04) return 9;
  if (totalMovement <= 0.06) return 8;
  if (totalMovement <= 0.08) return 7;
  if (totalMovement <= 0.1) return 6;
  return Math.max(5, 10 - totalMovement * 50); // 确保最低分为 5
}
...
function updateStabilityScores(
  ...
  if (prevWristPositions.left && prevWristPositions.right) {
    ...
    armStabilityScore = Math.max(0, armStabilityScore - totalMovement * 10);
  }
