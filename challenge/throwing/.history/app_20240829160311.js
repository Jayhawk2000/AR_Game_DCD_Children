...
function startCountdown() {
  if (countdownTimer > 0) {
    ...
    info.innerHTML += "<br><small>Make sure your full body is visible</small>";
    countdownTimer--;
...
function checkBalancePose(landmarks) {
  ...
  const oneFootRaised = Math.abs(leftAnkle.y - rightAnkle.y) > 0.15;
  ...
  if (oneFootRaised && armsRaised) {
    ...
    if (balancePoseDetected) {
      ...
      if (!oneFootRaised) {
        info.textContent = "Raise one foot higher";
      } else if (!armsRaised) {
        if (leftArmRaised && !rightArmRaised) {
          info.textContent = "Raise your right arm";
        } else if (rightArmRaised && !leftArmRaised) {
          info.textContent = "Raise your left arm";
        } else {
