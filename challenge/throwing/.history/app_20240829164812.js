...
function checkBalancePose(landmarks) {
  // Ensure footInfo and armInfo elements exist before trying to update them
  if (!footInfo || !armInfo) {
    console.error("Foot or arm info elements are not available.");
    return;
  }
  ...
  console.log("Checking balance pose:", {
    oneFootRaised,
    leftArmStatus,
    rightArmStatus,
  });
...
function onResults(results) {
  ...
  if (results.poseLandmarks && isDetectionStarted) {
    ...
    console.warn("Pose landmarks not detected or detection not started.");
  }
