...
function checkThrowingPose(landmarks) {
  if (!isThrowingStarted) {
    if (checkStartingPosition(landmarks)) {
      ...
      } else if (currentTime - startingPositionHoldTime >= STARTING_POSITION_HOLD_DURATION) {
...
function showResultPage(totalScore, scores) {
  ...
  document.getElementById("footStepScore").textContent = scores.footStepScore.toFixed(1);
  document.getElementById("armSpeedScore").textContent = scores.armSpeedScore.toFixed(1);
