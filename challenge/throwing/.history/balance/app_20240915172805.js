...
function checkThrowingPose(landmarks) {
  if (!isThrowingStarted) {
    if (checkStartingPosition(landmarks)) {
      ...
      info.textContent = "Starting position detected. Begin throwing!";
    }
    return;
  }
...
function checkStartingPosition(landmarks) {
  ...
  return isSideOn && isArmBehindHead;
}
...
function checkFootStep(landmarks) {
  ...
  switch(throwingPhase) {
    case 'preparation':
      ...
      if (throwingScores.preparation > 15) throwingPhase = 'propulsion';
      break;
    case 'propulsion':
      throwingScores.propulsion = checkPropulsionPhase(landmarks);
      if (throwingScores.propulsion > 20) throwingPhase = 'release';
      break;
    case 'release':
      throwingScores.release = checkReleasePhase(landmarks);
      if (throwingScores.release > 15) throwingPhase = 'followThrough';
      break;
    case 'followThrough':
      throwingScores.followThrough = checkFollowThroughPhase(landmarks);
      if (throwingScores.followThrough > 10) {
...
function checkSideOnStance(landmarks) {
  ...
  return shoulderDiff > 0.1 ? 1 : shoulderDiff / 0.1;
}

function checkArmBackwardMovement(landmarks) {
  const shoulder = landmarks[12]; // 右肩
  const wrist = landmarks[16]; // 右手腕
  return wrist.x < shoulder.x ? 1 : 0;
}
...
function checkFootStep(landmarks) {
  ...
  const rightAnkle = landmarks[28];
  const stepDiff = Math.abs(leftAnkle.x - rightAnkle.x);
  return stepDiff > 0.1 ? 1 : stepDiff / 0.1;
}
...
function checkArmForwardMovement(landmarks) {
  const shoulder = landmarks[12]; // 右肩
  const wrist = landmarks[16]; // 右手腕
  return wrist.x > shoulder.x ? 1 : 0;
}

function checkFollowThroughMovement(landmarks) {
  const shoulder = landmarks[12]; // 右肩
  const wrist = landmarks[16]; // 右手腕
  return wrist.y > shoulder.y ? 1 : 0;
}
