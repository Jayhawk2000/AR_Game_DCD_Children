...
function checkKeepBalance(landmarks) {
  ...
  if (oneFootLifted && armsRaised) {
    ...
    const remainingTime = Math.max(0, Math.ceil((BALANCE_DURATION - elapsedTime) / 1000));
    
    if (elapsedTime < BALANCE_DURATION) {
      instructionText.textContent = `Great! Keep your balance for ${remainingTime} more seconds.`;
    } else {
      instructionText.textContent = "Excellent! You've maintained balance for 10 seconds.";
    }
  } else {
    balanceStartTime = 0;
    if (!oneFootLifted) {
