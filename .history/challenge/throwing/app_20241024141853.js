...
function initCamera() {
  ...
  return navigator.mediaDevices
    ...
    .then((stream) => {
      ...
      video.style.transform = "scaleX(-1)"; // 添加水平翻转
...
function setupCanvas() {
  ...
  canvasElement.style.transform = "scaleX(-1)"; // 添加水平翻转
  
  // 设置画布上下文
  canvasCtx = canvasElement.getContext("2d");
  canvasCtx.translate(canvasElement.width, 0); // 添加这行
  canvasCtx.scale(-1, 1); // 添加这行
}
